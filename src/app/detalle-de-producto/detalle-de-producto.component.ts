...
import { ExchangeRateService } from 'src/app/exchange-rate.service';
...
export class DetalleDeProductoComponent implements OnInit {
  ...
  public usdPrice: number | null = null;
  public eurPrice: number | null = null;



  constructor(private exchangeService: ExchangeRateService, private carritoService: CarritoService, private productosService: ProductosService, private activatedRoute: ActivatedRoute, private dataSharingService: DataSharingService) {
...
async ngOnInit() {
  ...
  // Mostrar imagen por defecto si hay fotos
  if (this.producto.fotos.length >= 0) {
    this.seleccionarImagen(0);
  }

  // Convertir precio a otras monedas
  this.exchangeService.getExchangeRates().subscribe(data => {
    const rates = data.conversion_rates;
    this.usdPrice = Number(this.producto.precio) * rates.USD;
    this.eurPrice = Number(this.producto.precio) * rates.EUR;
